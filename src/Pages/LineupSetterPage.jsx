...
<div className="print-only">
  ...
  {coachSummaryGames.map((game) => {
    const lineup = coachSummaryLineups[pk(game.id)]
    if (!lineup) return null

    const playersInGame = activePlayers.filter((p) =>
      lineup.availablePlayerIds.includes(pk(p.id))
    )
    ...
    return (
      <div key={game.id} style={{ pageBreakAfter: 'always' }}>
